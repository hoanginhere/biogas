...
# Define parameters
L, U = 28.5, 30.5  # Range for ETIS
alpha1 = 0.09
alpha2 = 0.9
beta = 0.01

#Define Temperature and Humidity Information
T_tout = [15, 13.3, 13.2, 16.85 , 18.5, 21.95, 21.4, 21.25, 21.1, 21.2, 21.3, 21.9, 20.5, 21.3, 23.1, 24.0, 24.9, 25.4, 25.9, 27.3, 28.7, 29.5, 30.3, 30.65, 31, 31.95, 32.9, 32.55, 32.2, 32.05, 31.9, 30.75, 29.6, 28.65, 27.7, 27.3, 27.1, 26.9, 16.3, 15.7, 14.95, 14.2, 14.5 ,14.8, 15, 12.6, 13.4, 12.3]
...
def objective_function(T_Tin):
    ...
    for i in range(1, len(T_Tin)):
        e_t[i] = e_T(T_Tin[i], T_Tin[i-1], T_tout[i-1])
        if (e_t[i] < 0.5) :
            e_t[i] = 0
            T_Tin[i] = epsilon*T_Tin[i-1]+(1-epsilon)*T_tout[i-1]
        else:
            if (e_t[i] > 20) :
                e_t[i] = 20
                T_Tin[i] = epsilon*T_Tin[i-1]+(1-epsilon)*T_tout[i-1]  
...
positions = np.random.uniform(28, 38, (num_particles, num_dimensions))
